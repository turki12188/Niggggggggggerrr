...
function Move-OldSpicetifyFolder {
  ...
  process {
    if (Test-Path -Path $spicetifyOldFolderPath) {
      Write-Host -Object 'Moving the old SpFTex folder...' -NoNewline
...
function Get-Spicetify {
  ...
  begin {
    ...
    if ($v) {
      ...
      Write-Host -Object 'Fetching the latest SpFTex version...' -NoNewline
  ...
  process {
    Write-Host -Object "Downloading SpFTex v$targetVersion..." -NoNewline
    $Parameters = @{
...
function Add-SpicetifyToPath {
  ...
  param ()
  begin {
    Write-Host -Object 'Making SpFTex available in the PATH...' -NoNewline
...
function Install-Spicetify {
  ...
  param ()
  begin {
    Write-Host -Object 'Installing SpFTex...'
  }
  process {
    $archivePath = Get-Spicetify
    Write-Host -Object 'Extracting SpFTex...' -NoNewline
    ...
    Write-Host -Object 'SpFTex was successfully installed!' -ForegroundColor 'Green'
  }
}
#endregion Functions
...
if (-not (Test-Admin)) {
  ...
  if ($choice -eq 0) {
    Write-Host -Object 'SpFTex installation aborted' -ForegroundColor 'Yellow'
    Pause
    exit
  }
}
...
$choice = $Host.UI.PromptForChoice('', "`nDo you also want to install SpFTex Marketplace? It will become available within the Spotify client, where you can easily install themes and extensions.", $choices, 0)
if ($choice -eq 1) {
  Write-Host -Object 'SpFTex Marketplace installation aborted' -ForegroundColor 'Yellow'
}
else {
  Write-Host -Object 'Starting the SpFTex Marketplace installation script..'
  $Parameters = @{
